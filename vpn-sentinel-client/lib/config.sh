...
load_config() {
	...
	# Normalize API_PATH to include a leading slash
	if [ -z "$API_PATH" ]; then
		API_PATH="/api/v1"
	fi
	case "$API_PATH" in
	...
	# Allow overriding defaults from environment for faster tests and runtime tuning
	# Support both VPN_SENTINEL_* and legacy INTERVAL/TIMEOUT env names
	TIMEOUT=${VPN_SENTINEL_TIMEOUT:-${TIMEOUT:-30}}
	INTERVAL=${VPN_SENTINEL_INTERVAL:-${INTERVAL:-300}}
