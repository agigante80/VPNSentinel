#!/usr/bin/env bash
# Lightweight compatibility shim for the Python health-monitor.
# This script exists only to preserve tests and CI that call the legacy
# `health-monitor.sh` interface. It forwards commands to the Python
# `health-monitor.py` when available, otherwise returns a harmless exit.

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PYTHON_MONITOR="${SCRIPT_DIR}/health-monitor.py"

if [ -x "$(command -v python3)" ] && [ -f "$PYTHON_MONITOR" ]; then
  # Prefer the Python monitor; forward all args.
  exec python3 "$PYTHON_MONITOR" "$@"
else
  # If Python monitor is not present, provide conservative behavior for --stop
  # and similar commands so tests don't fail unexpectedly.
  case "$1" in
    --stop)
      # Nothing to stop; exit successfully so callers treat it as idempotent.
      exit 0
      ;;
    --help|-h)
      echo "health-monitor shim: forwards to health-monitor.py if present"
      exit 0
      ;;
    *)
      # No-op default.
