...
def test_build_payload_prints_valid_json(monkeypatch):
    ...
    p = Popen(["python3", "vpn-sentinel-client/lib/payload.py", "--build-json"], stdout=PIPE)
    out, _ = p.communicate()
...
def test_post_payload_writes_capture_file(tmp_path):
    ...
    p1 = Popen(["python3", "vpn-sentinel-client/lib/payload.py", "--build-json"], stdout=PIPE, env=env)
    out, _ = p1.communicate()
    p2 = Popen(["python3", "vpn-sentinel-client/lib/payload.py", "--post"], stdin=PIPE, env=env)
    p2.communicate(out)
    assert cap.exists()
