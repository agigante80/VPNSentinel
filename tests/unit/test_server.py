...
class TestServerFunctions(unittest.TestCase):
    ...
    def test_get_server_info_invalid_ip(self, mock_get):
        ...
        mock_response = Mock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"ip": "invalid-ip", "city": "", "region": "", "country": "", "org": ""}
        mock_get.return_value = mock_response
...
class TestKeepAliveEndpoint(unittest.TestCase):
    ...
    def test_keepalive_same_ip_detection(self):
        ...
        is_same_ip = client_ip != "unknown" and server_ip != "Unknown" and client_ip == server_ip
        ...
        is_different_ip = (
            different_client_ip != "unknown" and server_ip != "Unknown" and different_client_ip == server_ip
        )
    ...
    def test_health_endpoint_no_auth_required(self):
        ...
        for endpoint in endpoints:
            with self.subTest(endpoint=endpoint):
                ...
                response = self.health_client.get(endpoint, headers={"Authorization": "Bearer fake-token"})
                self.assertEqual(response.status_code, 200)
