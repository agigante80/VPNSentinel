...
class TestGetPortConfig:
    ...
    def test_get_port_config_partial_override(self):
        ...
        with patch.dict("os.environ", {"VPN_SENTINEL_SERVER_API_PORT": "9999"}, clear=True):
            config = get_port_config()
...
class TestRunFlaskApp:
    ...
    def test_run_flask_app_with_tls(self):
        ...
        with patch.dict(
            "os.environ",
            {"VPN_SENTINEL_TLS_CERT_PATH": "/path/to/cert.pem", "VPN_SENTINEL_TLS_KEY_PATH": "/path/to/key.pem"},
        ):
    ...
    def test_run_flask_app_ssl_load_error_exits(self):
        ...
        with patch.dict(
            "os.environ",
            {"VPN_SENTINEL_TLS_CERT_PATH": "/invalid/cert.pem", "VPN_SENTINEL_TLS_KEY_PATH": "/invalid/key.pem"},
        ):
...
class TestServerUtilsIntegration:
    ...
    def test_port_config_and_run_integration(self):
        ...
        with patch.dict("os.environ", {"VPN_SENTINEL_SERVER_API_PORT": "5555"}, clear=True):
            config = get_port_config()
