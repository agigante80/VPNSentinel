...
from vpn_sentinel.common.config import _sanitize_client_id, generate_client_id, load_config
...
class TestLoadConfig:
    ...
    def test_load_config_new_env_overrides_legacy(self):
        ...
        env = {"TIMEOUT": "45", "VPN_SENTINEL_TIMEOUT": "60", "INTERVAL": "450", "VPN_SENTINEL_INTERVAL": "600"}
        config = load_config(env)
    ...
    def test_load_config_debug_case_insensitive(self):
        ...
        test_cases = [("true", True), ("True", True), ("TRUE", True), ("false", False), ("anything-else", False)]
    ...
    def test_load_config_server_url_construction(self):
        ...
        for base, path, expected_url in test_cases:
            env = {"VPN_SENTINEL_URL": base, "VPN_SENTINEL_API_PATH": path}
            config = load_config(env)
    ...
    def test_config_complete_workflow(self):
        ...
        with patch("vpn_sentinel.common.config.time.time", return_value=1699721234.567):
            with patch("vpn_sentinel.common.config.random.randint", return_value=123456):
                env = {"VPN_SENTINEL_URL": "https://vpn.example.com", "VPN_SENTINEL_TIMEOUT": "45"}
                config = load_config(env)
