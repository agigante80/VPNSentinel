...
def test_monitor_heartbeats_and_shutdown(tmp_path):
    ...
    proc = subprocess.Popen(
        [PYTHON, str(MONITOR)], stdout=subprocess.PIPE, stderr=subprocess.STDOUT, env=env, text=True
    )
    ...
    try:
        ...
        shutdown_lines = read_lines_until(
            proc, lambda l: "Monitor stopped" in l or "shutting down gracefully" in l, timeout=5.0
        )
        assert any(
            "Monitor stopped" in l or "shutting down gracefully" in l for l in shutdown_lines
        ), f"no shutdown confirmation in output: {shutdown_lines}"
...
def test_main_reads_environment():
    ...
    with patch.dict("os.environ", {"VPN_SENTINEL_MONITOR_INTERVAL": "60", "VERSION": "2.0.0", "COMMIT_HASH": "abc123"}):
