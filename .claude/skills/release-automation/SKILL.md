...
- VERSION_SOURCE=git (tag-derived). Lane A (the file-bump gate) is moot: there is no version file to
  bump in a PR, so skip it. See references/python-tag-derived.md.
- Lane C (auto-release on merge) is the lane: every green merge to main with unreleased commits since
  the latest tag gets the next tag plus release. This is the requested auto-release-on-merge behavior.
